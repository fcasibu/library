let myLibrary = [];
class Book {
  constructor(title, author, currentPage, totalPage, status) {
    this.title = title;
    this.author = author;
...
const filterBookDropDown = document.querySelector("#filter-books");
...
function generateStatus(parent, index, book) {
  const status = document.createElement("td");
  status.classList.add("status");
  status.textContent = book[index].status;
  switch (book[index].status) {
    case "Reading":
...
function displayBooks(book) {
  ...
  for (let i = 0; i < book.length; i++) {
    ...
    generateStatus(tableRow, i, book);
